// 在新建模块（如 packages/static-sites/src/handler.ts）中
import { Handler, Types, param } from 'hydrooj';
import { readFileSync, existsSync } from 'fs';
import { join, resolve } from 'path';

// 静态站点根目录（根据实际路径调整）
const STATIC_ROOT = resolve(__dirname, './static-site');
...
export class StaticSiteHandler extends Handler {
    ...
    async get({ path }) {
        ...
        const filePath = path ? join(STATIC_ROOT, path) : join(STATIC_ROOT, 'index.html');
...
export async function apply(ctx: Context) {
    // 配置静态子站根路径（如 /static-site 对应外部静态包）
    ctx.Route('static_site', '/static-site/*path', StaticSiteHandler);
}