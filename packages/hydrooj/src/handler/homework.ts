...
class HomeworkMainHandler extends Handler {
  ...
  async get(domainId: string, group = "", page = 1) {
    ...
    for (const tdoc of tdocs) {
      const cal = {
        ...tdoc,
        url: this.url("homework_detail", { tid: tdoc.docId }),
      };
...
class HomeworkDetailHandler extends Handler {
  ...
  async get(domainId: string, tid: ObjectId, page = 1) {
    ...
    const [ddocs, dpcount, dcount] = await this.paginate(
      discussion.getMulti(domainId, {
        parentType: this.tdoc.docType,
        parentId: this.tdoc.docId,
      }),
      page,
      "discussion"
    );
    ...
    this.response.body.tdoc.content = this.response.body.tdoc.content
      ...
      .replace(/="file:\/\//g, `="./${this.tdoc.docId}/file/`);
    if (
      (contest.isNotStarted(this.tdoc) ||
        (!tsdoc?.attend && !contest.isDone(this.tdoc))) &&
      !this.user.own(this.tdoc) &&
    ...
    if (tsdoc) {
      if (tsdoc.attend && !tsdoc.startAt && contest.isOngoing(this.tdoc)) {
        await contest.setStatus(domainId, tid, this.user._id, {
          startAt: new Date(),
        });
  ...
  async postAttend({ domainId }) {
    ...
    if (contest.isDone(this.tdoc))
      throw new HomeworkNotLiveError(this.tdoc.docId);
...
class HomeworkEditHandler extends Handler {
  ...
  async get(domainId: string, tid: ObjectId) {
    ...
    const extensionDays = tid
      ? Math.round(
          (tdoc.endAt.getTime() - tdoc.penaltySince.getTime()) /
            (Time.day / 100)
        ) / 100
      : 1;
    const beginAt = tid
  ...
  async postUpdate(
    ...
    const beginAt = moment.tz(
      `${beginAtDate} ${beginAtTime}`,
      this.user.timeZone
    );
    if (!beginAt.isValid())
      throw new ValidationError("beginAtDate", "beginAtTime");
    const penaltySince = moment.tz(
      `${penaltySinceDate} ${penaltySinceTime}`,
      this.user.timeZone
    );
    if (!penaltySince.isValid())
      throw new ValidationError("endAtDate", "endAtTime");
    const endAt = penaltySince.clone().add(extensionDays, "days");
    if (beginAt.isSameOrAfter(penaltySince))
      throw new ValidationError("endAtDate", "endAtTime");
  ...
  async postDelete(domainId: string, tid: ObjectId) {
    ...
    await Promise.all([
      ...
      storage.del(
        tdoc.files?.map((i) => `contest/${domainId}/${tid}/${i.name}`) || [],
        this.user._id
      ),
    ]);
...
export async function apply(ctx) {
  ...
  ctx.Route(
    "homework_files",
    "/lession/:tid/file",
    HomeworkFilesHandler,
    PERM.PERM_VIEW_HOMEWORK
  );
  ctx.Route(
    "homework_file_download",
  ...
  ctx.inject(["scoreboard"], ({ Route }) => {
    Route(
      "homework_scoreboard",
      "/lession/:tid/scoreboard",
      ContestScoreboardHandler,
      PERM.PERM_VIEW_HOMEWORK_SCOREBOARD
    );
    Route(
      "homework_scoreboard_view",
      "/lession/:tid/scoreboard/:view",
      ContestScoreboardHandler,
      PERM.PERM_VIEW_HOMEWORK_SCOREBOARD
    );
  });
