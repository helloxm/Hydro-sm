...
export class HomeHandler extends Handler {
    ...
    async getContest(domainId: string, limit = 10) {
        ...
        const rules = Object.keys(contest.RULES).filter(
            (i) => !contest.RULES[i].hidden,
        );
        const groups = (
            await user.listGroup(
                domainId,
    ...
    async getStarredProblems(domainId: string, limit = 50) {
        ...
        const psdocs = await ProblemModel.getMultiStatus(domainId, {
            uid: this.user._id,
            star: true,
        })
            .sort('_id', 1)
    ...
    async get({ domainId }) {
        ...
        for (const column of info) {
            ...
            for (const name in column) {
                ...
                const func = `get${camelCase(name).replace(/^[a-z]/, (i) =>
                    i.toUpperCase(),
                )}`;
...
class HomeSecurityHandler extends Handler {
    ...
    async postDeleteToken({}, tokenDigest: string) {
        ...
        throw new InvalidTokenError(
            token.TYPE_TEXTS[token.TYPE_SESSION],
            tokenDigest,
        );
    }
    ...
    getAuthnHost() {
        return system.get('authn.host')
      && this.request.hostname.includes(system.get('authn.host'))
    ...
    async postEnableAuthn({}, name: string) {
        ...
        if (
            this.user._authenticators.find(
                (c) => c.credentialID.buffer.toString() === id.toString(),
            )
        ...
        this.user._authenticators.push({
            ...
            regat: Date.now(),
            authenticatorAttachment:
        this.args.result.authenticatorAttachment || 'cross-platform',
        });
    ...
    async postDisableAuthn({}, id: string) {
        const authenticators = this.user._authenticators?.filter(
            (c) => Buffer.from(c.credentialID.buffer).toString('base64') !== id,
        );
...
class HomeSettingsHandler extends Handler {
    ...
    async post(args: any) {
        ...
        const settings = args.category === 'course'
...
class HomeAvatarHandler extends Handler {
    ...
    async post({}, input: string) {
        if (input) {
            ...
            await storage.put(
                `user/${this.user._id}/.avatar${ext}`,
                file.filepath,
                this.user._id,
            );
            // TODO: cached avatar
            await user.setById(this.user._id, {
                avatar: `url:/file/${this.user._id}/.avatar${ext}`,
            });
...
class UserChangemailWithCodeHandler extends Handler {
    ...
    async get(domainId: string, code: string) {
        ...
        if (!tdoc || tdoc.uid !== this.user._id) {
            throw new InvalidTokenError(
                token.TYPE_TEXTS[token.TYPE_CHANGEMAIL],
                code,
            );
        }
...
class HomeDomainHandler extends Handler {
    ...
    async get(domainId: string, all: boolean) {
        ...
        for (const ddoc of res) {
            ...
            const bManage: Boolean = udoc.hasPerm(PERM.PERM_EDIT_DOMAIN)
    ...
    async postStar({}, id: string, star = false) {
        if (star) {
            await user.setById(this.user._id, {
                pinnedDomains: [...this.user.pinnedDomains, id],
            });
        } else {
            user.setById(this.user._id, {
                pinnedDomains: this.user.pinnedDomains.filter((i) => i !== id),
            });
        }
        this.back({ star });
    }
...
class HomeDomainCreateHandler extends Handler {
    ...
    async post(
        ...
        const domainId = await domain.add(
            id,
            this.user._id,
            name,
            bulletin,
        ...
        await Promise.all([
            ...
            push
                ? user.setById(this.user._id, undefined, undefined, {
                    pinnedDomains: domainId,
                })
                : Promise.resolve(),
        ]);
...
export function apply(ctx: Context) {
    ...
    ctx.Route(
        'user_changemail_with_code',
        '/home/changeMail/:code',
